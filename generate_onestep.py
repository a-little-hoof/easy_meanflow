...
def main(network_pkl, outdir, subdirs, seeds, class_idx, max_batch_size, num_fid_samples, sigma_G, device=torch.device('cuda')):
    ...
    for batch_seeds in tqdm.tqdm(rank_batches, unit='batch', disable=(dist.get_rank() != 0)):
        ...
        sigma_min = 0.002
        sigma_max = 1
        images = (latents-net(latents, sigma_min*torch.ones([latents.shape[0], 1, 1, 1], device=latents.device), class_labels, t=sigma_max*torch.ones([latents.shape[0], 1, 1, 1], device=latents.device), augment_labels=torch.zeros(latents.shape[0], 9).to(latents.device)))
